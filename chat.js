...
function setupWebSocket() {
    ...
    socket.on('response', function(data) {
	    ...
	    // If this is an assistant response (not user input), ensure the avatar is speaking it
            if (chunkString.includes('Assistant:') || !chunkString.includes('User:')) {
                // The actual speaking is handled by the backend via speakWithQueue
                // We just need to ensure the avatar is connected and ready
                if (!speechSynthesizerConnected) {
                   console.log("Speech synthesizer not connected, attempting to reconnect...")
                   connectAvatar()
                }
            }
            ...
            } else if (data.eventType === 'SPEECH_SYNTHESIZER_CONNECTED') {
